  ...
  var doc = w.document, docElem = doc.documentElement, mediastyles = [], rules = [], appendedEls = [], parsedSheets = {}, resizeThrottle = 30, head = doc.getElementsByTagName("head")[0]
    ...
    for (var i in mediastyles) {
      if (mediastyles.hasOwnProperty(i)) {
        ...
        if (!thisstyle.hasquery || (!minnull || !maxnull) && (minnull || currWidth >= min)
    ...
    var qs = styles.replace(respond.regex.keyframes, "").match(respond.regex.media), ql = qs
                                                                                          && qs.length
                                                                                          || 0;
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      if (!!href && isCSS && !parsedSheets[href]) {
        if (sheet.styleSheet && sheet.styleSheet.rawCssText) {
          ...
          if (!/^([a-zA-Z:]*\/\/)/.test(href) && !base || href.replace(RegExp.$1, "").split("/")[0]
                                                          === w.location.host) {
