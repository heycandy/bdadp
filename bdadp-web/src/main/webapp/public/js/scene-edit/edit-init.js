       ...
       function (ETab, EForm, Edges, Emonitor, Dialog, Ajax, Search, Copy) {
         ...
         return function () {
           ...
           var _create = function (sceneId, sceneName) {
             ...
             if (sceneEdit.length > 0 && settings.currentSceneTab.exist(sceneId)) {
               ...
               var _html = '<section id="' + _id + '" class="content sceneEdit-sec"' +
                           ...
                           + '<li class="selectexec"><span class="selectIcon glyphicon glyphicon-triangle-bottom" style="font-size: 15px;"></span>&nbsp;<span>'                  // restore execution
                           ...
                           '<li><span class="ion-help-circled" style="font-size: 15px;"></span><span>'
                           + $.i18n.prop("d_span_edit_help")
                           + '</span></li><li class="ion-trash-a" style="display: none" title="清除状态"><span>'
                           + $.i18n.prop("d_span_edit_eliminate") + '</span></li>' +
                           '</ul>' +
                           '</div></div>' +
                           '<div><ul class="select-dropdown dropdown-menu" style="display: none"><div class="user-down"></div><ul class="checkedData" style="list-style: none"></ul><ul style="padding: 0px;background-color: #e6e6e6;"><li class = "clear-li" style="text-align: center;list-style: none;padding: 0px"><a class="clearSelected">'
                           + $.i18n.prop("reminder_span_allRemove")
                           + '</a></li></ul></ul></div>' +
               ...
               loadPlatte(sceneId, "html/scene-edit/dndPlatte.html", function () {
                 var diagram = new DFlow().dflowInit(_diagramId, _myOverview, "contextMenu",
                       ...
                       if (isShow != "none" && e.Hq.type != "mousemove" && e.targetObject.Qn
                           ...
                           if (KeyData.indexOf(node.data.key) != -1) {
                             new Dialog().initConfirmDialog(function () {
                                 ...
                                 , {"message": $.i18n.prop('d_edit_repeatSelecting')}, function () {
                                   $('#cancelBtn').remove();
                                   $('.modal-dialog .modal-body .messContent span:eq(0)').attr(
                                       'class',
                             ...
                             $("#" + sceneId + " ul.checkedData").append(
                                 '<li class = "selectedComponents"  style="list-style: none" key='
                                 + node.data.key + '><img width = 25 src= ' + node.data.img + '>'
                                 + node.data.text
                                 + '<span class="glyphicon glyphicon-remove pull-right" title="删除" style="color: #e66864;padding: 9px;cursor: pointer"></span></li>')
                           }
                         } else {
                           $("#" + sceneId + " ul.checkedData").append(
                               '<li class = "selectedComponents"  style="list-style: none" key='
                         ...
                         //$("#"+sceneId+" ul.checkedData").append('<li class =
                         // "selectedComponents"  style="list-style: none"
                         // key='+node.data.key+'><img width = 25 src=
                         // '+node.data.img+'>'+node.data.text+'<span class="glyphicon
                         // glyphicon-remove pull-right" title="删除" style="color: #e66864;padding:
                         // 9px;cursor: pointer"></span></li>')
                         ...
                         getAjaxComponentArg(componentId, function (response) {
                           if (response.resultCode == 0) {
                             ...
                             tabPanelArr.push({
                                              ...
                                              });   // tabPanelArr add the current show
                                                    // tabPanelClass
                             _tabPanelArr.setTabPanelArr(tabPanelArr);
                             var eForm = new EForm();
                             $('#' + sceneId + " .attribute-panel").append(
                                 eForm.create(tabPanelClass,
                                              formId,
                                              attriTabId,
                                              descriTabId,
                                              res.result,
                             ...
                             if (data.resultCode == 0) {
                               new Dialog().initConfirmDialog(function () {
                                   ...
                                   , null, function () {
                                     ...
                                     $('.modal-footer').remove();
                                     $('.modal-dialog>.modal-content>.modal-header').prepend(
                                         "<span class='glyphicon glyphicon-fullscreen' style='margin-left: 90%;font-size: 13px;cursor: pointer'></span>")
                                     $('.modal-dialog>.modal-content>.modal-header .modal-title').css(
                                         {
                                           float: 'left'
                                         })
                                     $('.modal-dialog>.modal-content>.modal-header span').click(
                                         function () {
                                           ...
                                           if ($(this).attr('class')
                                             ...
                                             $(this).attr('class', 'ion-arrow-shrink').css(
                                                 'font-size', '18px')
                                             $('.task_log').css({
                                                                  'height': $(
                                                                      '.modal-dialog>.modal-content').height()
                                                                            - 30 + 'px'
                                                                })
                                             ...
                                             $(this).attr('class',
                                                          'glyphicon glyphicon-fullscreen').css({
                                                                                                  'font-size': '13px',
                                                                                                  'margin-left': '90%',
                                                                                                  'cursor': 'pointer'
                                                                                                })
                                     ...
                                     if (data.result instanceof Array) {
                                       ...
                                       $('.task_log').append(
                                           '<div id =log_graph_temp style="width: 450px;height: 400px;margin: 0 auto;text-align: center;">');
                                       var myChart = echarts.init(
                                           document.getElementById("log_graph_temp"));
                 ...
                 var drag = new DFlow(diagram[0].diagram).platteInit(sceneId, "edit-platte",
                                                                     diagram);
                 ...
                 new Copy().init(sceneId, diagram[0].diagram, function (data) {
                   data.forEach(function (_d) {
                     var srcSceneId = _d.node.formId.split('_')[0],
                         ...
                         thatKey = _d.pasternode.key, componentType = "component",
                         nodeText = _d.node.text;
                     if (componentId.indexOf('_') != -1) {
                       var _type = componentId.substring(componentId.indexOf('_') + 1);
                       if (_type == "scenario") {
                         ...
                         for (var j = 0; j < configArr.length; j++) {   //traverse each element of form
                           ...
                           if (param_type == "text") {
                             ...
                             $('#' + formId + ' #' + param_id
                               ...
                               if (param_type == "radio" && $(element).attr('value') == $.grep(
                                       _d.data,
                                       function (n) {
                                         return (n.name
                                                 == param_id)
                 ...
                 getAjaxGraphArg(sceneId, function (response) {
                   if (response.resultCode == 0) {
                     ...
                     if (result == null) {         //create the new scene when edit a new one
                       ...
                       if (modelJson && modelJson.nodeDataArray && modelJson.nodeDataArray.length
                                                                   > 0) {
                 ...
                 settings.InterNation(['.edit_sceneComponent', '.edit_dataExtraction',
                                       ...
                                       '.d_span_dnd_delete'],
                                      ['d_span_sceneComponent', 'd_span_dataExtraction',
                                       'd_span_dataImport',
                 ...
                 if (type == 'expr') {
                   ...
                   for (var j = 0; j < option.length; j++) {
                     if (option[j].args.length == 1) {
                       optionArray.push({
                                          name: option[j].name + "(" + option[j].args[0].dataType
                                                + " " + option[j].args[0].name + ")"
                                        });
                     } else if (option[j].args.length == 2) {
                       optionArray.push({
                                          name: option[j].name + "(" + option[j].args[0].dataType
                                                + " " + option[j].args[0].name + ","
                                                + option[j].args[1].dataType + " "
                                                + option[j].args[1].name + ")"
                                        });
                     }
                   }
                 }
           ...
           function addZoom(sceneId, myDiagram) {
             $('#' + sceneId
               + " .myZoom").append(
                 '<table class="table table-no-bordered" ondragstart="return false">'
                 +
           ...
           function selectExcute(sceneId) {
             $("#" + sceneId + " .dropzone-top li.selectexec").click(function () {
               ...
               if (flagIcon) {
                 $("#" + sceneId + " .dropzone-top span.selectIcon").removeClass(
                     'glyphicon-triangle-bottom').addClass('glyphicon-triangle-top')
               } else {
                 $("#" + sceneId + " .dropzone-top span.selectIcon").removeClass(
                     'glyphicon-triangle-top').addClass('glyphicon-triangle-bottom')
               }
               flagIcon = !flagIcon
           ...
           function save(sceneId, diagram, graph_id) {
             ...
             $("#" + sceneId + " .dropzone-top li.ion-archive").click(function () {
               ...
               if (tabPanelArr instanceof Array) {
                 ...
                 var edges = $.parseJSON(myDiagram.model.toJSON()).linkDataArray.map(
                     function (ele) {
                       return {'from': ele.from, 'to': ele.to}
                     });
                 var vertices = $.parseJSON(myDiagram.model.toJSON()).nodeDataArray.map(
                     function (ele) {
                       return ele.key
                     });
                 for (var i = 0; i < tabPanelArr.length; i++) {
                   ...
                   var key = tabPanelArr[i].tabelPanel.substr(
                       tabPanelArr[i].tabelPanel.lastIndexOf("_")
                   ...
                   _graph_vertexs_node.task.task_name =
                       $('#tabDesc_' + formId).find('label').html();
                   _graph_vertexs_node.task.task_desc =
                       $('#tabDesc_' + formId).find('textarea').val();
           ...
           function setDropZone(myDiagram, sceneId) {
             ...
             $('#' + sceneId + " .close-panel").each(function (index, dom) {
               ...
               $(dom).append(
                   '<img src="img/component/side-slider/r-right.png" style="cursor: pointer" />');
               $(dom).find('img').css('margin-left', '-10px').css('margin-top',
                                                                  (bodyHeight / 2 - 80)).on({
                                                                                              mouseout: function () {
                                                                                                if ($(this).attr(
                                                                                                        'src')
                                                                                                    == "img/component/side-slider/r-right-focus.png") {
                                                                                                  $(this).attr(
                                                                                                      'src',
                                                                                                      "img/component/side-slider/r-right.png");
                                                                                                } else {
                                                                                                  if ($(this).attr(
                                                                                                          'src')
                                                                                                      != 'img/component/side-slider/r-right.png') {
                                                                                                    $(this).attr(
                                                                                                        'src',
                                                                                                        "img/component/side-slider/r-left.png");
                                                                                                  }
                                                                                                }
                                                                                              },
                                                                                              mouseover: function () {
                                                                                                if ($(this).attr(
                                                                                                  ...
                                                                                                  $(this).attr(
                                                                                                      'src',
                                                                                                      "img/component/side-slider/r-left-focus.png");
                                                                                                }
                                                                                              }
                                                                                            });
               ...
               if (sceneId != tJson[i].scenario_id) {
                 var _src = tJson[i].scenario_col,
                     _img = '<img draggable="false" data-pid="' + tJson[i].scenario_id
                            ...
                            +
                            '<span style="vertical-align: 15%;">' + tJson[i].scenario_name
                            + '</span>';
           ...
           function platteOuterCss(sceneId) {
             ...
             $('#' + sceneId + ' .collapseOne').on('show.bs.collapse', function () {
               if (event.target.className == "ion-chevron-down") {
                 $('#' + sceneId
                   + " .panel-heading:eq(0) .panel-title li[class*='ion-chevron']:first").each(
                     function (index,
                               dom) {
                       if (0 == index) {
             ...
             $('#' + sceneId + ' .collapseTwo').on('show.bs.collapse', function () {
               if (event.target.className == "ion-chevron-down") {
                 $('#' + sceneId
                   + " .panel-group>.panel>.panel-heading:eq(2) .panel-title li[class*='ion-chevron']:first").each(
                     function (index,
                               dom) {
                       if (0 == index) {
                         $(dom).attr("class", "ion-chevron-up");
             ...
             $('#' + sceneId + ' .collapseThree').on('show.bs.collapse', function () {
               if (event.target.className == "ion-chevron-down") {
                 $('#' + sceneId
                   + " .panel-heading.customer .panel-title li[class*='ion-chevron']:first").each(
                     function (index,
                               dom) {
                       if (0 == index) {
                         $(dom).attr("class", "ion-chevron-up");
           ...
           function platteTopCss(sceneId) {
             ...
             $('#' + sceneId + ' .scenario-extraction').on('show.bs.collapse', function () {
               $('#' + sceneId
                 + " .scenario-assembly .scenario-extr-title li[class*='ion-arrow']:first").each(
                   function (index,
                             dom) {
                     if (0 == index) {
           ...
           function platteMiddleCss(sceneId) {
             ...
             $('#' + sceneId + ' .collapseThree>.panel-body:first').slimScroll({
                                                                                 ...
                                                                                 //  height:
                                                                                 // $('.sceneEdit-sec').height()/3-40
                                                                               });
             //1 data-extraction panel --click
             $('#' + sceneId + ' .data-extr-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             //2 import --click
             $('#' + sceneId + ' .data-import-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             //4 common --click
             $('#' + sceneId + ' .data-common-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             $('#' + sceneId + ' .data-transfer').on('show.bs.collapse', function () {
               $('#' + sceneId
                 + " .data-transfer-title li[class*='ion-arrow']:first").each(
                   function (index, dom) {
                     if (0 == index) {
                       $(this).attr("class", "ion-arrow-down-b");
                     }
           ...
           function platteBottomCss(sceneId) {
             // data-resource --click
             $('#' + sceneId + ' .data-resource-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             // data-handling --click
             $('#' + sceneId + ' .data-handling-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             // data-output --click
             $('#' + sceneId + ' .data-output-title li.ion-arrow-right-b').each(
                 function (index, dom) {
                   $(dom).on('click', function (e) {
             ...
             $('#' + sceneId + ' .data-resource').on('show.bs.collapse', function () {
               $('#' + sceneId
                 + " .customer-assembly .data-resource-title li[class*='ion-arrow']:first").each(
                   function (index, dom) {
                     if (0 == index) {
                       $(this).attr("class", "ion-arrow-down-b");
                     }
             ...
             $('#' + sceneId + ' .data-handling').on('show.bs.collapse', function () {
               $('#' + sceneId
                 + " .customer-assembly .data-handling-title li[class*='ion-arrow']:first").each(
                   function (index, dom) {
                     if (0 == index) {
                       $(this).attr("class", "ion-arrow-down-b");
                     }
             ...
             $('#' + sceneId + ' .data-output').on('show.bs.collapse', function () {
               $('#' + sceneId
                 + " .customer-assembly .data-output-title li[class*='ion-arrow']:first").each(
                   function (index, dom) {
                     if (0 == index) {
                       $(this).attr("class", "ion-arrow-down-b");
                     }
