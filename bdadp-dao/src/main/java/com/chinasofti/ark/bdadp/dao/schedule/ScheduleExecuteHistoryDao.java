...
public interface ScheduleExecuteHistoryDao extends JpaRepository<ScenarioExecuteHistory, String> {
    ...
    @Query("select new com.chinasofti.ark.bdadp.util.dto.MonitorDTO(s.taskName ,s.unionId.executionId, s.unionId.taskId, s.scenarioId,s.executeStatus ,s.startTime,s.endTime,s.executeProgress ,s.createTime) from com.chinasofti.ark.bdadp.entity.schedule.ScenarioExecuteHistory s where  s.createTime >= :searchDate and s.scenarioId = s.unionId.taskId order by s.createTime desc")
    Page<MonitorDTO> getPageJobsBySearchday(Pageable page,
                                            @Param(value = "searchDate") Date searchDate);

    @Query("select new com.chinasofti.ark.bdadp.util.dto.MonitorDTO(s.taskName ,s.unionId.executionId, s.unionId.taskId, s.scenarioId,s.executeStatus ,s.startTime,s.endTime,s.executeProgress ,s.createTime) from com.chinasofti.ark.bdadp.entity.schedule.ScenarioExecuteHistory s where  s.executeStatus = :executeStatus and s.createTime >= :searchDate and s.scenarioId in (:scenarioIds) and s.scenarioId = s.unionId.taskId order by s.createTime desc")
    ...
    @Query("select new com.chinasofti.ark.bdadp.util.dto.MonitorDTO(s.taskName ,s.unionId.executionId, s.unionId.taskId, s.scenarioId,s.executeStatus ,s.startTime,s.endTime,s.executeProgress ,s.createTime) from com.chinasofti.ark.bdadp.entity.schedule.ScenarioExecuteHistory s where  s.unionId.executionId = :executionId and   s.scenarioId = :scenarioId and s.scenarioId != s.unionId.taskId order by s.createTime asc")
    ...
    @Query("select new com.chinasofti.ark.bdadp.util.dto.MonitorDTO(s.taskName ,s.unionId.executionId, s.unionId.taskId, s.scenarioId,s.executeStatus ,s.startTime,s.endTime,s.executeProgress ,s.createTime) from com.chinasofti.ark.bdadp.entity.schedule.ScenarioExecuteHistory s where  s.createTime >= :searchDate and s.scenarioId = s.unionId.taskId and s.scenarioId in (:scenarioIds)  order by s.createTime desc")
