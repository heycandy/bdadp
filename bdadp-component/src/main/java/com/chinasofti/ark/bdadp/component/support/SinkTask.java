...
import com.chinasofti.ark.bdadp.component.api.sink.StreamSinkAdapter;
...
public class SinkTask extends SimpleTask<SinkComponent> implements ChannelInputable {
  ...
  protected void execute() throws Exception {
    ...
    StreamSupport.stream(entrySet.entrySet()).forEach(entry -> {
      ...
      } else if (dataType == DataType.STREAM && this.obj instanceof StreamSinkAdapter) {
        input = this.options().as(SparkScenarioOptions.class)
            .union(channels).output();
        ((StreamSinkAdapter) this.obj).apply((StreamData) input);
