...
/**
 * <Detect encoding .> Copyright (C) <2009>  <Fluck,ACC http://androidos.cc/dev> <p> This program is
 * free software: you can redistribute it and/or modify it under the terms of the GNU General Public
 * License as published by the Free Software Foundation, either version 3 of the License, or (at
 * your option) any later version. <p> This program is distributed in the hope that it will be
...
public class EncodingDetect {
    ...
    public static String getJavaEncodeHDFS(FileSystem fs, FileStatus status, String filePath) {
        BytesEncodingDetect s = new BytesEncodingDetect();
        String fileCode = BytesEncodingDetect.javaname[s.detectEncodingHDFS(fs, status, filePath)];
        return fileCode;
    }
...
class BytesEncodingDetect extends Encoding {
    ...
    //for HDFS
    public int detectEncodingHDFS(FileSystem fs, FileStatus status, String testfilePath) {
        //文件为HDFS
        FSDataInputStream chinesefile = null;
        byte[] rawtextt = null;
        try {
            chinesefile = fs.open(new Path(testfilePath));
            rawtextt = new byte[(int) status.getLen()];

            chinesefile.read(rawtextt);
            chinesefile.close();
        } catch (Exception e) {
            System.err.println("Error: " + e);
        }
        return detectEncoding(rawtextt);
    }
